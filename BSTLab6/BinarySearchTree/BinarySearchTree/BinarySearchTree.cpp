...
bool BinarySearchTree::recAdd(Node*& ptr, int newNodeValue){
...
bool BinarySearchTree::recRemove(Node*& ptr, int valueToRemove){
	if (ptr == NULL) {
	...
	if (ptr->getData == valueToRemove){
		...
		// If there is only a left child
			...
			Node* temp = ptr;                                                  //TODO: I dont think i'm setting the temps correctly
		...
		// If there are children on both right and left
			...
			InorderPredecessor(ptr, ptr->getLeftChild);
		}
	}
	return true;
}
...
void BinarySearchTree::InorderPredecessor(Node*& n1, Node*& n2){
	if (n2->getRightChild != NULL){
		InorderPredecessor(n1, n2->getRightChild);
	}
...
void BinarySearchTree::eraseBST(Node* root){
